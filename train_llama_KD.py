...
class KDTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        labels = teacher_outputs.get('logits')
        outputs = model(**inputs)
        logits = outputs.get('logits')
        loss = torch.norm(torch.softmax(logits.squeeze() / 0.1, dim=-1) - torch.softmax(labels.squeeze() / 0.1, dim=-1), p=1)
...
class ModelArguments:
    ...
    model_name_or_path: Optional[str] = field(
        ...
        metadata={
            "help": (
                "The student model checkpoint for weights initialization. Don't set if you want to train a model from scratch."
            )
        },
    )
